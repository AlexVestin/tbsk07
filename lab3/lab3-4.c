...
GLfloat specularExponent[] = { 1.0, 100.0, 200.0, 150.0, 200.0 };
...
void drawWorld() {
	...
	glUniform1f(glGetUniformLocation(noTexProgram, "specularExponent"), specularExponent[3]);
	...
	trans = T(4.5, 9.25, 0);
	glUniform1f(glGetUniformLocation(noTexProgram, "specularExponent"), specularExponent[4]);
	for (int j = 0; j < 4; j++) {
...
void init(void)
{
	...
	noTexProgram = loadShaders("lab3-4.vert", "lab3-4_no_tex.frag");
	noShaProgram = loadShaders("lab3-4.vert", "lab3-4_no_shadow.frag");
	...
	// We do the same for the textureless shader.
	glUseProgram(noTexProgram);
	glUniform3fv(glGetUniformLocation(noTexProgram, "lightSourcesDirPosArr"), 4, &lightSourcesDirectionsPositions[0].x);
	glUniform3fv(glGetUniformLocation(noTexProgram, "lightSourcesColorArr"), 4, &lightSourcesColorsArr[0].x);
	glUniform1iv(glGetUniformLocation(noTexProgram, "isDirectional"), 4, isDirectional);
...
void display(void)
{
	...
	glUniform3fv(glGetUniformLocation(noTexProgram, "camPos"), 1, &camPos);
	...
	// -- This shader won't use the camera position. --
