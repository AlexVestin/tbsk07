...
Model* GenerateTerrain(TextureData *tex)
{
	...
	for (x = 0; x < tex->width; x++)
		for (z = 0; z < tex->height; z++)
		{
			...
			uint16_t L = x > 0 ? tex->imageData[( (x-1) + z * tex->width) * (tex->bpp/8)]  : 0;
			uint16_t R = x < tex->width - 1 ? tex->imageData[( (x+1) + z * tex->width) * (tex->bpp/8)]  : 0;
			uint16_t B = z > 0 ? tex->imageData[(x + (z-1) * tex->width) * (tex->bpp/8)] : 0;
			uint16_t T = z < tex->height - 1 ? tex->imageData[(x + (z+1) * tex->width) * (tex->bpp/8)]  : 0;
