...
Model* GenerateTerrain(TextureData *tex)
{
	...
	for (x = 0; x < tex->width; x++)
		for (z = 0; z < tex->height; z++)
		{
			...
			vertexArray[(x + z * tex->width)*3 + 1] = getValue(tex, x, z, heightScale);
	...
	/**
	 The indices are lined up as  	                             
	   tl 6         tr                                           
		...
		1 +------+ 4                                            
	   bl        3  br             
		where 1,2,3 make up the "left" triangle and 4,5,6 the "right".
	*/         
...
void display(void)
{
	...
	glUniform3f(glGetUniformLocation(program, "lightPos"), 5, 5, 5);	
	...
	moveSphere();
	glUseProgram(sphereProgram);
	mat4 sphereMdl = T(spherePosition.x, spherePosition.y, spherePosition.z);
	...
	glUniform3f(glGetUniformLocation(sphereProgram, "lightPos"), 5, 5, 5);
	DrawModel(sphere, sphereProgram, "inPosition", "inNormal", NULL);
